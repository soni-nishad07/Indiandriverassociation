...
<div class="registration-container">
    ...
    <form class="registration-form" action="admin/register_driver.php" method="POST" enctype="multipart/form-data">
        ...
        <div class="form-group">
            <label>Signature</label>
            <p>Upload your signature (scanned file):</p>
            <input type="file" id="signature_file" name="signature_file" accept=".jpg, .jpeg, .png" onchange="clearCanvas()">
            <p>Or sign directly on your mobile device:</p>
            <div id="signature_pad" style="border: 1px solid #ccc; width: 100%; height: 200px; touch-action: none;"></div>
            <button type="button" id="clear_signature">Clear</button>
...
<script>
    ...
    function clearCanvas() {
        ctx.clearRect(0, 0, canvas.width, canvas.height); // Clear the canvas when file input changes
    }

    // Convert canvas to data URL and store it in the hidden input
    document.querySelector('form').addEventListener('submit', (event) => {
        const signatureFile = document.getElementById('signature_file').files.length > 0;
        const signatureData = canvas.toDataURL();

        // Check if both signature methods are used
        ...
        // Store signature data only if it's drawn
        if (!signatureFile) {
            document.getElementById('signature_data').value = signatureData;
        } else {
            document.getElementById('signature_data').value = ''; // Clear signature data if using uploaded file
        }
    });
</script>
