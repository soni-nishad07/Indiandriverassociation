...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $phone_check_query = "SELECT * FROM driver_info WHERE phone = ?";
    ...
    $insert_query = "INSERT INTO driver_info (driver_mode, driver_name, driver_photo, phone, dl_number, area_postal_code, address, vehicle_type, signature_file , registration_date , staff_id) 
                     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), ?)";

    if ($stmt = $conn->prepare($insert_query)) {
        $stmt->bind_param("ssssssssss", $driver_mode, $driver_name, $photo_filename, $phone, $dl_number, $area_postal_code, $address, $vehicle_type, $signature_file, $staff_id);

        if ($stmt->execute()) {
            echo "<script>alert('Driver registered successfully.');
